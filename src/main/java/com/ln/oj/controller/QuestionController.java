...
import com.ln.oj.model.dto.question.*;
...
import com.ln.oj.model.vo.QuestionVO;
...
public class QuestionController {
    ...
    @GetMapping("/ai")
    public BaseResponse<String> aiChat(Long questionId, HttpServletRequest request) {
        User loginUser = userService.getLoginUser(request);
        redisLimiter.doRateLimit("aiChat_"+loginUser.getId());
        Integer coin = loginUser.getCoin();
        if (coin <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR,"题币不足");
        }
        ...
        String content = "现在有一道程序设计题目，要求程序运行时间不超过"+timeLimit+"ms，程序占用内存不超过"+memoryLimit+"KB，程序占用堆栈不超过"+stackLimit+
                "KB，以上要求不要在代码中实现，题目内容为：" + questionContent + "，示例答案为：\n"+ answer + "\n" +
                "请你对该题的解题思路并分析示例答案的思路，然后根据题目内容给出其他的Java语言的答案";
        String res = AIUtil.chat(content,"星火");
        log.info("AI响应：{}",res);
        loginUser.setCoin(coin - 1);
    ...
    public BaseResponse<Long> doQuestionSubmit(@RequestBody QuestionSubmitAddRequest questionSubmitAddRequest,
        ...
        redisLimiter.doRateLimit("doQuestionSubmit_"+loginUser.getId());
        if (questionSubmitAddRequest == null || questionSubmitAddRequest.getQuestionId() <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        String method = questionSubmitAddRequest.getMethod();
        if ("ai".equals(method) && (loginUser.getCoin() <= 0)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR,"题币不足");
        }
        long questionSubmitId = questionSubmitService.doQuestionSubmit(questionSubmitAddRequest, loginUser);
        if ("ai".equals(method)) {
            loginUser.setCoin(loginUser.getCoin() - 1);
            userService.updateById(loginUser);
        }
